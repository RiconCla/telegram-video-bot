...
async function downloadInstagram(url, userId) {
    ...
    try {
        ...
        if (response.data) {
            ...
            if (data.media && Array.isArray(data.media) && data.media.length > 0) {
                // ===== НОВАЯ ЛОГИКА: Проверяем ВСЕ элементы массива =====
                ...
                // Если несколько медиа (карусель) - собираем все URL
                const mediaUrls = [];
                ...
                if (mediaUrls.length > 0) {
                    // Определяем тип: если есть видео - смешанный тип, иначе - image
                    ...
                    // Возвращаем массив URL (как для TikTok слайдшоу)
                    return {
                        success: true,
                        type: finalType,
                        url: mediaUrls // массив всех URL
                    };
                }
            }
    ...
    // Метод 2: Прямой запрос к Instagram (улучшенный для каруселей)
    try {
        ...
        if (shortcode) {
            ...
            // Ищем JSON данные в HTML (Instagram встраивает данные в script теги)
            const jsonMatch = html.match(/<script type="application\/ld\+json">({.*?})<\/script>/);

            // Пытаемся найти все display_url в HTML
            const displayUrls = [];
            ...
            while ((match = displayUrlRegex.exec(html)) !== null) {
                ...
                // Проверяем, что это уникальный URL и не добавляем дубликаты
                if (!displayUrls.includes(imageUrl)) {
                    displayUrls.push(imageUrl);
                }
            }

            // Ищем video_url для видео
            ...
            // Если найдено несколько изображений (карусель)
            if (displayUrls.length > 1) {
                log('success', `Method 2 successful: carousel with ${displayUrls.length} images`, userId);
                return {
                    success: true,
                    type: 'image',
                    url: displayUrls // массив URL
                };
            }

            // Если найдено одно изображение
            if (displayUrls.length === 1) {
    ...
    // Метод 3: Старый embed метод (улучшенный для каруселей)
    try {
        ...
        // Ищем ВСЕ display_url для каруселей
        const displayUrls = [];
        const displayUrlRegex = /"display_url":"([^"]+)"/g;
        let match;
